...
namespace Phones\StatProviders\GsmArenaComBasemarkOSIIBundle\Service;

use Doctrine\ORM\EntityManager;
use Phones\FrontEndBundle\Service\QueryHelper;
use Phones\PhoneBundle\Entity\BasemarkOSII;
...
class MainDownloader
{
    ...
    private function curlData($link)
    {
        ...
        if ($doc != null) {
            ...
            foreach ($trElements as $phoneData) {
                ...
                $basemarkOSIIRating = !empty($values[1]) ? (int)$values[1] : null;

                if (!empty($phoneName) && !empty($basemarkOSIIRating)) {
                    $stat = new BasemarkOSII();
                    $stat->setProviderId($this->provider);
                    $stat->setOriginalPhoneName($phoneName);
                    $stat->setBasemarkOsIiRating($basemarkOSIIRating);
    ...
    /**
     * @param BasemarkOSII[] $phoneStats
     */
    private function saveStats($phoneStats)
    {
        foreach ($phoneStats as $stat) {
            $this->entityManager
                ->getRepository('PhonesPhoneBundle:BasemarkOSII')
                ->save($stat);
        }
    }
